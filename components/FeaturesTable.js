...
const fetchFeatures = async () => {
  ...
  return result.data.listFeatures.items.sort(sortByTime);
};
    ...
    const createSub = API.graphql(graphqlOperation(onCreateFeature)).subscribe({
      next: ({ value }) => {
        queryClient.setQueryData(featuresQueryKey, (current) => {
          ...
          console.log("create index", toCreateIndex);
          if (toCreateIndex !== -1) {
            return current;
          }
          console.log("what happened?", current, value.data.onCreateFeature);
    ...
    const updateSub = API.graphql(graphqlOperation(onUpdateFeature)).subscribe({
      next: ({ value }) => {
        queryClient.setQueryData(featuresQueryKey, (current) => {
          ...
          console.log("toUpdateIndex Update", toUpdateIndex);

          if (toUpdateIndex === -1) {
            return [...current, value.data.onUpdateFeature];
          }

          current[toUpdateIndex] = value.data.onUpdateFeature;

          console.log("test", current, value.data.onUpdateFeature);
          return current;
        });
      },
    });
